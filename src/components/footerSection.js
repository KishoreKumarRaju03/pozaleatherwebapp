import '../style/componentStyle/footerSectionStyle.css';
import pozaLogo from '../asset/logo/poza-logo.png';

const FooterSection = () => {
    return(
        <>
            <div className="footer-container">
                <div className="footer-section">
                    <div className="footer-section-content">
                        <div className="footer-section-customer-care">
                            <h4>Customer Care</h4>
                            <ul>
                                <li>Contact Us</li>
                                <li>shipping</li>
                                <li>FAQ</li>
                                <li>Catalog Unsubscribe</li>
                            </ul>
                        </div>
                        <div className="footer-section-about-us">
                            <h4>About Us</h4>
                            <ul>
                                <li>About Poza Leather</li>
                                <li>Our Leather</li>
                                <li>Testimonials</li>
                                <li>affiliates</li>
                                <li>Production Unit</li>
                            </ul>
                        </div>
                        <div className="footer-section-Services">
                            <h4>Services</h4>
                            <ul>
                                <li>Events</li>
                                <li>Gifts</li>
                                <li>Corporate Orders</li>
                                <li>Personalization</li>
                            </ul>
                        </div>
                        <div className="footer-section-Connect-with-us">
                            <div className="footer-section-Connect-with-us-icons">
                                <div className="footer-section-Connect-with-us-header">
                                    <h4>Connect with us...</h4>
                                </div>
                                <div className="footer-section-Connect-with-us-content">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="white" d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"/></svg>
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="white" d="M512 256C512 114.6 397.4 0 256 0S0 114.6 0 256C0 376 82.7 476.8 194.2 504.5V334.2H141.4V256h52.8V222.3c0-87.1 39.4-127.5 125-127.5c16.2 0 44.2 3.2 55.7 6.4V172c-6-.6-16.5-1-29.6-1c-42 0-58.2 15.9-58.2 57.2V256h83.6l-14.4 78.2H287V510.1C413.8 494.8 512 386.9 512 256h0z"/></svg>
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="white" d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z"/></svg>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='footer-section-end'>
                        <div className='footer-section-end-container'>
                            <div className='footer-section-end-logo'>
                                <img src={pozaLogo} alt='poza-logo' />
                            </div>
                            <div className='footer-section-end-copyright'>
                                <span>© 2022 POZA Leather</span>
                            </div>
                            <div className='footer-section-end-policy'>
                                <span>Privacy Policy</span>
                                <span>Terms & Conditions</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </>
    );
};

export default FooterSection;